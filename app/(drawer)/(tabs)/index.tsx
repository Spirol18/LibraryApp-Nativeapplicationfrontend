...
function ContinuePlayingSection({ userEmail }: { userEmail: string }) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_BASE_URL}/getcurrentbook?email=${encodeURIComponent(userEmail)}`,
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        <Box className="p-4 rounded-2xl border border-outline-100 bg-background-0 shadow-soft-2">
          <HStack space="md" className="items-center">
            ...
            <VStack className="flex-1">
              ...
              <Text
                size="xs"
                className="text-typography-500 mb-2"
                numberOfLines={1}
              >
                {book.author}
              </Text>
    ...
    <Box className="flex-1 bg-background-0">
        ...
        <Box className="flex-1 px-4 pt-4">
          ...
          <HStack className="mb-8 items-center justify-between">
            <HStack className="items-center gap-3">
              ...
              <VStack>
                ...
                <Text size="xs" className="text-typography-500">
                  Listen to Literary fiction Audio book
                </Text>
              </VStack>
            </HStack>
            ...
            <Pressable
              ...
              className="p-2.5 rounded-full border border-[#313538] bg-[#202325]"
            >
          ...
          >
            ...
            >
              <Pressable
                className="w-4/5 max-w-[340px] bg-[#202325] border border-[#313538] rounded-3xl p-6 items-center shadow-lg"
                onPress={(e) => e.stopPropagation()}
              >
                <Box className="w-16 h-16 bg-[#61dafb] rounded-full items-center justify-center mb-4 shadow-md">
                  <Text className="text-[#151718] font-bold text-3xl">
                    {userInitial}
                  </Text>
                </Box>
                ...
                <Text size="sm" className="text-[#9BA1A6] mb-8 font-medium">
                  {userEmail}
                </Text>
          ...
          <Box
            className={`
        flex-row items-center px-4 py-3 rounded-2xl mb-6
        border
        ${isFocused ? "border-primary-500" : "border-outline-200"}
      ...
      `}
        ...
        `}
              ...
              underlineColorAndroid="transparent" // removes blue underline (Android)
              selectionColor="#6366F1" // custom cursor color
              cursorColor="#6366F1" // Android cursor
            />
          ...
          <HStack className="justify-between items-center mb-4">
            ...
            <HStack space="sm">
              ...
              >
                <Text
                  className={`text-xs font-medium ${sortBy === "date" ? "text-black" : "text-typography-500"}`}
                >
                  Newest
                </Text>
              </Pressable>
              ...
              >
                <Text
                  className={`text-xs font-medium ${sortBy === "alphabet" ? "text-black" : "text-typography-500"}`}
                >
                  A-Z
                </Text>
              </Pressable>
          ...
          <VStack space="md">
            {filteredAndSortedBooks.map((book) => (
              ...
              >
                <Box
                  className={`p-4 rounded-xl border border-outline-100 ${colorScheme === "dark" ? "bg-background-50" : "bg-white"} shadow-sm`}
                >
                  <HStack space="md">
                    {book.image ? (
                      <Image
                      ...
                      <Box
                        className={`w-20 h-28 rounded-md ${book.coverColor} shadow-sm`}
                      />
                    )}
                    <VStack className="flex-1 py-1 justify-between">
                      <Box>
                        <Heading
                          size="lg"
                          className="text-typography-900 leading-tight mb-1"
                        >
                          {book.title}
                        </Heading>
                        <Text
