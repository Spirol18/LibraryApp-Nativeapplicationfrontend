...
import { X, Play, Pause, SkipBack, SkipForward, RotateCcw, RotateCw } from "lucide-react-native";
...
function PlayPopup({
      ...
      try {
        ...
        if (!currentTrack || currentTrack.id !== chapter.id) {
          ...
          await TrackPlayer.add({
            ...
            artwork: "https://picsum.photos/id/145/600/600", // Spotify-like placeholder
  ...
  const togglePlayback = async () => {
    ...
    currentState === State.Playing ? await TrackPlayer.pause() : await TrackPlayer.play();
        ...
        <Box className="w-[85%] bg-[#121212] rounded-[20px] p-5 shadow-xl border border-white/10">
          ...
          <Box className="flex-row justify-between items-center mb-3">
            ...
            <TouchableOpacity
              ...
              style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.1)', justifyContent: 'center', alignItems: 'center' }}
          ...
          <Box className="mb-1">
            ...
            <Box className="flex-row justify-between px-1">
              ...
              <Text className="text-[#b3b3b3] text-[10px]">{formatTime(progress.position)}</Text>

              <Text className="text-[#b3b3b3] text-[10px]">{formatTime(progress.duration)}</Text>
